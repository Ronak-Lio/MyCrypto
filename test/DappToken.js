...
contract("DappToken", function (accounts) {
  ...
  it("approves tokens for delegate transfer", function () {
    return DappToken.deployed()
      ...
      .then(function (receipt) {
        ...
        assert.equal(
          receipt.logs[0].args._spendinger,
          accounts[1],
  ...
  it("transfers token ownership", function () {
    return DappToken.deployed()
      .then(function (instance) {
        tokenInstance = instance;
        // Test `require` statement first by transferring something larger than the sender's balance
        console.log("In transfer ", accounts[1]);
        return tokenInstance.transfer.call(accounts[1], 250000, {
          from: accounts[0],
        });
      })
  ...
  it("handles deelegated token transfer", function () {
    return DappToken.deployed()
      .then(function (instance) {
        tokenInstance = instance;
      ...
      .then(assert.fail)
      .catch(function (error) {
        assert(error.message.indexOf("revert") >= 0, "cannot tansfer");
        return tokenInstance.transferFrom(fromAccount, toAccount, 80, {
          from: spendingAccount,
        })
      }).then(assert.fail).catch(function (error) {
         assert(error.message.indexOf("revert") >= 0, "cannot transfer greater than the approved vslue")
        ...
        assert.equal(receipt.logs[0].event.args._from,fromAccount,'logs the FROM Acccount')
        assert.equal(receipt.logs[0].event.args._to,toAccount,'logs the TO Acccount')
        assert.equal(receipt.logs[0].event.args._value,10,'logs the transfer amount')
